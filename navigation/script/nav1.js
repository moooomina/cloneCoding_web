...
console.log(gnb, lnb, nav_bg)

//메뉴에 마우스 올리면 서브메뉴 보이는 기능
/* for(let l of lnb){
...
//메뉴1에 마우스를 올리면 애니메이션효과로 서브메뉴가 보이게 하는 법
/* for(let l of lnb){
    l.style.height = '0'
...
//메뉴에 마우스 올렸을 때 모든 서브메뉴가 보이게 하는 법
...
for(let g of gnb){
    //2. gnb에 마우스이벤트시 lnb를 모두 제어하고 싶다면
    for(let l of lnb){
        //3.이벤트 대상 확인 후 "g" 이벤트 함수 생성
        g.addEventListener('mouseover',()=>{
        //4. 위 이벤트 조건이 만족되었을 때 lnb모두 출력
        //모두 출력하는 변수 대상확인 "l" 관련 속성 적용
